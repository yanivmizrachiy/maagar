...
function filters() {
  const g = ['all', ...vals('grade')], c = ['all', ...vals('primary_category')], t = ['all', ...vals('document_type')];
...
function filtered() {
  ...
  return S.files.filter(f => {
    ...
    if (!q) return true;
    const hay = [title(f), f.file_name, f.path, f.author, f.year, f.primary_category, f.document_type, ...(f.tags || []), ...topics(f)].join(' ').toLowerCase();
...
function files() {
  const arr = filtered().sort((a, b) => (primary(a) + title(a)).localeCompare(primary(b) + title(b), 'he'));
  $('ttl').textContent = S.q ? 'תוצאות חיפוש' : 'קבצים במאגר';
  $('meta').textContent = count(arr.length);
  if (!arr.length) {
    $('app').className = '';
  ...
  arr.forEach(f => { const k = primary(f); if (!groups.has(k)) groups.set(k, []); groups.get(k).push(f); });
  $('app').className = 'groups';
  $('app').innerHTML = [...groups].map(([k, items]) => `<section class="group"><div class="ghead"><span>${esc(k)}</span><span>${count(items.length)}</span></div><div class="grid">${items.map(card).join('')}</div></section>`).join('');
...
function card(f) {
  const u = url(f);
  const e = ext(f);
  const labels = [G[f.grade], C[f.primary_category], T[f.document_type], e ? '.' + e : ''].filter(Boolean);
...
function open(id) {
  ...
  $('ms').textContent = canDownload ? 'צפייה מוטמעת · הורדה ישירה זמינה' : 'צפייה מוטמעת';
  $('mo').href = u;
  $('mo').target = '_blank';
